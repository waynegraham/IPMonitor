    ...
    with_config do|_app, _host, db, _user|
      File.open("#{Rails.root}/data/archived_statuses_#{db}_#{Time.now.to_i}.csv", 'w') do |f|
        ...
        Resource.find_each do |resource|
          # grab the last 200 statuses for this resource
          last_ten = resource.statuses.last(200)

          # grab everything but the last 200
          old_statuses = Status.where(resource_id: resource.id).where.not(id: last_ten)
